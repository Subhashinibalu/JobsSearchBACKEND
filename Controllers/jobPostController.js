...
export const postJob = async (req, res) => {
    ...
    const {company,role,skillsRequired,location,salary,link,email} = req.body
    const newJob = new jobPost({company,role,skillsRequired,location,salary,link,email})
    try {
...
user.map(user => 
    {
       ...
       const usermongodb = user.skills.includes("MongoDB");
        ...
        if((userhtml&&html)==true || (usercss&&css)==true || (userjavascript&&javascript)==true|| (usermongodb&&mongodb)==true 
        || (usermysql&&mysql)==true || (userds&&ds)==true || (userjava&&java)==true || (userc&&c)==true || (userpython&&python)==true){
